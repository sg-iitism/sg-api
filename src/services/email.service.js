...
const sendResetPasswordEmail = async (to, token) => {
  ...
  const resetPasswordUrl = `${config.frontendAdminPanel}/reset-password?token=${token}`;
...
const sendVerificationEmail = async (to, token) => {
  ...
  const verificationEmailUrl = `${config.frontendAdminPanel}/verify-email?token=${token}`;
  const text = `Dear user,
